...
public class ChannelLWJGLOpenAL extends Channel {
  ...
  public boolean preLoadBuffers(LinkedList<byte[]> bufferList) {
    ...
    for (int i = 0; i < bufferList.size(); i++) {
      byteBuffer = (ByteBuffer)BufferUtils.createByteBuffer(((byte[])bufferList.get(i)).length).put(bufferList.get(i)).flip();
      try {
  ...
  public int feedRawAudioData(byte[] buffer) {
    ...
    if (checkALError())
      return -1; 
    if (this.attachedSource != null && this.attachedSource.channel == this && this.attachedSource.active())
      if (!playing()) {
        AL10.alSourcePlay(this.ALSource.get(0));
        checkALError();
      }  
...
/* Location:              C:\Users\Joona\Downloads\Cupid.jar!\paulscode\sound\libraries\ChannelLWJGLOpenAL.class
