...
public class UserConnection {
  ...
  public void sendRawPacket(ByteBuf packet, boolean currentThread) {
    Runnable act;
    if (this.clientSide) {
      act = (() -> getChannel().pipeline().context(Via.getManager().getInjector().getDecoderName()).fireChannelRead(packet));
    } else {
  ...
  public boolean exceedsMaxPPS() {
    ...
    if (conf.getMaxPPS() > 0 && 
      this.packetsPerSecond >= conf.getMaxPPS()) {
      disconnect(conf.getMaxPPSKickMessage().replace("%pps", Long.toString(this.packetsPerSecond)));
      return true;
    } 
    if (conf.getMaxWarnings() > 0 && conf.getTrackingPeriod() > 0)
      if (this.secondsObserved > conf.getTrackingPeriod()) {
        ...
        if (this.warnings >= conf.getMaxWarnings()) {
          disconnect(conf.getMaxWarningsKickMessage().replace("%pps", Long.toString(this.packetsPerSecond)));
          return true;
        } 
      }  
    return false;
  ...
  private void sendRawPacketToServerClientSide(ByteBuf packet, boolean currentThread) {
    Runnable act = () -> getChannel().pipeline().context(Via.getManager().getInjector().getEncoderName()).writeAndFlush(packet);
    if (currentThread) {
      act.run();
    } else {
      try {
  ...
  public void transformOutgoing(ByteBuf buf, Function<Throwable, Exception> cancelSupplier) throws Exception {
    if (!buf.isReadable())
      return; 
    transform(buf, this.clientSide ? Direction.INCOMING : Direction.OUTGOING, cancelSupplier);
  }
  
  public void transformIncoming(ByteBuf buf, Function<Throwable, Exception> cancelSupplier) throws Exception {
    if (!buf.isReadable())
      return; 
    transform(buf, this.clientSide ? Direction.OUTGOING : Direction.INCOMING, cancelSupplier);
  }
  ...
  private void transform(ByteBuf buf, Direction direction, Function<Throwable, Exception> cancelSupplier) throws Exception {
    ...
    try {
      ...
      throw (Exception)cancelSupplier.apply(ex);
    } 
...
/* Location:              C:\Users\Joona\Downloads\Cupid.jar\\us\myles\ViaVersion\api\data\UserConnection.class
